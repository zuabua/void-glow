...
if [ ! -d "$SDDM_SRC" ]; then
  ...
  # Three cases: file missing, [Theme] missing, Current= line exists.
  ...
  if [ ! -f "$SDDM_CONF" ]; then
    ...
    # [Theme] exists but no Current= line — insert it after the section header
...
# 9. STOW PACKAGES
# ─────────────────────────────────────────────────────────────────────
# Order matters slightly: theme first (palette SSOT), then everything
# that depends on it. In practice Stow is just symlinking, so order
# doesn't change correctness — but it makes failure diagnosis cleaner.
# ─────────────────────────────────────────────────────────────────────
...
for pkg in "${STOW_PKGS[@]}"; do
  ...
  info "stow: $pkg"
  # --no-folding keeps directories explicit (clearer for debug);
  # --restow is idempotent — re-links everything cleanly each run.
